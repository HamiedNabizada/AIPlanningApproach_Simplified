...
move = InstantaneousAction('move', b=Bot, l_from=Location, l_to=Location)
b = move.parameter('b')
l_from = move.parameter('l_from')
l_to = move.parameter('l_to')
move.add_precondition(connected(l_from, l_to))
move.add_precondition(at(b, l_from))
move.add_effect(at(b, l_from), False)
move.add_effect(at(b, l_to), True)
"""
...
pickup = InstantaneousAction('pickup', pb=PickingRobot, tr=TransportingRobot, ltr=Location, lpb=Location, tgpb=TransportingGood)
pb = pickup.parameter('pb')
tr = pickup.parameter('tr')
ltr = pickup.parameter('ltr')
lpb = pickup.parameter('lpb')
tgpb = pickup.parameter('tgpb')
pickup.add_precondition(at(tgpb, lpb)) # Transportgut muss an lpb sein
pickup.add_precondition(at(pb, lpb)) # PickingRobot muss auch da sein
pickup.add_precondition(connected(ltr, lpb)) #TransportRobot muss an einer Stelle sein, die direkt verbunden ist mit PickingPlace
pickup.add_precondition(at(tr, ltr))
pickup.add_precondition(Not(robot_occupied(tr))) #TransportRobot darf nichts anderes tragen
pickup.add_effect(at(tgpb, lpb), False) # Nochmal drüber nachdenken, Transportgut "verschwindet" in der Zwischenzeit vom Schachbrett, wenn TR beladen
pickup.add_effect(robot_occupied(tr), True)
"""
...
drop = InstantaneousAction('drop', pb1=PickingRobot, tr1=TransportingRobot, ltr1=Location, lpb1=Location, tgpb1=TransportingGood)
pb1 = drop.parameter('pb1')
tr1 = drop.parameter('tr1')
ltr1 = drop.parameter('ltr1')
lpb1 = drop.parameter('lpb1')
tgpb1 = drop.parameter('tgpb1')
drop.add_precondition(Not(at(tgpb1, lpb1)))
drop.add_precondition(at(pb1, lpb1))
drop.add_precondition(at(tr1, ltr1))
drop.add_precondition(connected(ltr1, lpb1))
drop.add_precondition(robot_occupied(tr1))
drop.add_effect(at(tgpb1, lpb1), True)
drop.add_effect(robot_occupied(tr1), False)
"""
...
problem.set_initial_value(at(transportGood, locations[10]), True)


problem.add_goal(at(transportGood, locations[-1]))
#problem.add_goal(at(availableRobots[0], locations[12]))
#problem.add_goal(at(availableRobots[1], locations[20]))
