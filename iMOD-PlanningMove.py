...
robot_at = unified_planning.model.Fluent('robot_at', BoolType(
), r=Robots, l=Location)  # Wo ist der Robi? True wenn Robi bei l ist dann True
connected = unified_planning.model.Fluent('connected', BoolType(
), l_from=Location, l_to=Location)  # Gibt es eine Verbindung zwischen l_from und l_to
occupied = unified_planning.model.Fluent(
    'occupied', RealType(0, 1), lo=Location)
...
move.add_condition(ClosedTimeInterval(
    StartTiming(), EndTiming()), Equals(occupied(l_to), 0))
...
availableRobots = []
for i in robotdata:
    availableRobots.append(unified_planning.model.Object(i['id'], Robots))
